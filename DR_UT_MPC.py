...
class MPC:
	def __init__(self, path, with_obs, L, Ts, N, xmax, xmin, umax, umin, Q_w, Qf_w, R_w, method = None):
		...
		self.npar = self.nx + 2*self.nx + self.nu + self.max_data*self.max_data*self.nx + self.max_data*self.nx + self.nx + self.nx + self.max_data*(self.nx+self.nu) + 1 + 3 + self.ny + self.ny*self.ny
					#goal   + Q, Qf     + R       + L                                   + alpha                 + var     + length  + X_train               +  rad + beta,theta,delta + obs_mean + obs_cov12
		self.alpha_cvar = 0.95

		self.delta = 0
		self.round_upto = 10

		if method=='drcvar':
			self.beta_cvar = np.sqrt(self.alpha_cvar/(1-self.alpha_cvar))
			self.theta = 0.1
		elif method=='cvar':
			self.beta_cvar = (np.sqrt(2*np.pi)*(np.exp(special.erfinv(2*self.alpha_cvar-1))**2)*(1-self.alpha_cvar))**(-1)
			self.theta = 0
		else:
			self.beta_cvar = 0
			self.theta = 0
		...
		try:
			self.solver = forcespro.nlp.Solver.from_directory("./DR_UT_MPC")
		except:
			print('Creating MPC')
			self.create_MPC()
	...
	def solve_MPC_backup(self, x_mean, x_obs, goal, obs_box_, car_box_, extent_o, extent_r, t):
		...
		tiled = np.tile(np.concatenate([self.Q_w, self.R_w, self.Qf_w, [r_o + r_r], x_obs[:self.ny]]),(self.N,1))
...
#		x0 = np.zeros((self.nx + self.nx*self.nx + self.nu, self.N))
		...
		if not isinstance(gp_out, tuple):
			...
			problem = {"x0": x0,
						"xinit": np.concatenate([self.u_old, x_mean, (1e-4*np.eye(self.nx)).reshape((self.nx*self.nx,))])}
			...
			tiled = np.tile(np.concatenate([self.Q_w, self.R_w, self.Qf_w,
											...
											[r_o + r_r, self.beta_cvar, self.theta, self.delta]]),(self.N,1))
...
#				temp[:, i] = output['x{0:0{size}d}'.format(i + 1, size=int(np.floor(np.log10(self.N)) + 1))]
	...
	def update_dist_np(self, X_s_bar, Q, w_m, w_c):
		...
		return mu, np.linalg.cholesky(Sigma + Q + 1e-4*np.eye(n)).T

	def update_dist_scalar(self, X_s_bar, w_m, w_c):
		mu = casadi.sum1(w_m * X_s_bar.T).T
		Sigma = casadi.sum1(w_c*((X_s_bar-mu)**2).T)
		return mu, casadi.sqrt(Sigma)
...
#
	...
	def dynamics(self, z, p):
		...
		u_new = deltau + u
#		P_12 = casadi.chol(P + 1e-4*casadi.SX.eye(self.nx))
	...
	def objective(self, z, p):
		...
		obj = casadi.dot((Q @ (x[:self.nx] - goal)), x[:self.nx] - goal) + casadi.dot((R @ deltau), deltau) + casadi.trace(Q @ P.T @ P)
		return obj
	...
	def objectiveN(self, z, p):
		...
		obj = casadi.dot((Qf @ (x[:self.nx] - goal)), x[:self.nx] - goal) + casadi.trace(Qf @ P.T @ P)
		return obj
	...
	def inequality(self, z,p):
		...
		P_o = casadi.reshape(p[self.nx + 2*self.nx + self.nu+self.max_data*self.max_data*self.nx + self.max_data*self.nx + self.nx + self.nx + self.max_data*(self.nx+self.nu) + 4 + self.ny:
							   self.nx + 2*self.nx + self.nu+self.max_data*self.max_data*self.nx + self.max_data*self.nx + self.nx + self.nx + self.max_data*(self.nx+self.nu) + 4 + self.ny + self.ny*self.ny], (self.ny, self.ny))

		x_joint = casadi.vertcat(x, x_o)
		P_joint = casadi.SX.zeros(self.ny+self.ny, self.ny+self.ny)
		P_joint[:self.ny, :self.ny] = P[:self.ny,:self.ny]
		P_joint[self.ny:, self.ny:] = P_o
		Sigma_p = self.create_sigma(x_joint, P_joint, self.gamma_l)
		Sigma_new = casadi.SX.sym('Sigma_new', 1, 2*(self.ny+self.ny)+1)

		for j in range(2*(self.ny+self.ny)+1):
			Sigma_new[j] = self.loss(Sigma_p[j][:self.ny], Sigma_p[j][self.ny:], r)

		mean, Sigma = self.update_dist_scalar(Sigma_new, self.w_ml, self.w_cl)

		beta = p[self.nx + 2*self.nx + self.nu+self.max_data*self.max_data*self.nx + self.max_data*self.nx + self.nx + self.nx + self.max_data*(self.nx+self.nu) + 1]
		theta = p[self.nx + 2*self.nx + self.nu+self.max_data*self.max_data*self.nx + self.max_data*self.nx + self.nx + self.nx + self.max_data*(self.nx+self.nu) + 2]
		delta = p[self.nx + 2*self.nx + self.nu+self.max_data*self.max_data*self.nx + self.max_data*self.nx + self.nx + self.nx + self.max_data*(self.nx+self.nu) + 3]

		return [mean + theta*casadi.sqrt(1+beta**2) - delta + beta*Sigma]
	...
	def create_MPC(self):
		...
		codeoptions = forcespro.CodeOptions('DR_UT_MPC')
	...
	def save(self):
		...
		np.save(self.path + 'obs_box.npy', np.array(self.obs_box))
		np.save(self.path + 'goals.npy', np.array(self.goals))
